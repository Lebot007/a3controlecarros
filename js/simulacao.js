...
export function criarSimulacao(ev) {
    ...
    // Spawn adaptativo: mantém o fluxo constante
    function proximoIntervalo() {
        const livres = vagasLivres();

        if (livres <= 1) return aleatorio(6, 12);  // quase cheio: segura
        if (livres >= 5) return aleatorio(2.5, 6); // vazio: enche mais rápido
    ...
    function tentarEntrada() {
        ...
        const carro = {
            ...
            vaga,
            saiEm: tempo + aleatorio(...CONFIG.permanencia),
        };

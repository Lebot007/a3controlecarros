...
/* ---------- veículo (cria sob demanda, vinculado a um cliente) ---------- */

export async function obterOuCriarVeiculo(placa, cor, modelo) {
    ...
    const { data: novo, error } = await supabase
        .from("veiculo")
        .insert([{ placa, cor, modelo, cliente_id: clienteAleatorio() }])
        .select("id")
        .single();
...
// Apaga os veículos da simulação (ON DELETE CASCADE também limpa a
// tabela movimentacao). Tabelas de referência (cliente, funcionario,
// vaga, tipo_vaga) são preservadas por serem cadastro fixo.
export async function limparMovimentacoes() {
    return await supabase.from("veiculo").delete().not("id", "is", null);
}

export async function registrarEntrada({ placa, cor, modelo, vaga, sessao }) {
    const veiculo_id = await obterOuCriarVeiculo(placa, cor, modelo);
    ...
    return await supabase
        .from("movimentacao")
        .insert([{ veiculo_id, vaga_id, funcionario_id, sessao, status: "ESTACIONADO" }]);
}
